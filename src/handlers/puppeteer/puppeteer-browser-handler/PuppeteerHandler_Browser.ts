...
export default class PuppeteerHandler_Browser
  implements IPuppeteerHandler_Browser
{
  ...
  static async createNewBrowser(
    ...
    const browser =
      await PuppeteerHandler_Browser.createPuppeteerObject_Browser(
        languageSavedValue,
        vpnHandler
      );
    return new this(
  ...
  private static async createPuppeteerObject_Browser(
    languageSavedValue: string,
    vpnHandler?: IProxy
  ) {
  ...
  async createNewPage(
    ...
    if (pageExistWithId) {
      throw new Error(
        `${file} - ${fn} - page already exist with same id = ${pageId}`
      );
    }
    ...
    this._pageList[pageId] = new PuppeteerHandler_Page(
      page,
      removePageFromParentList,
      this.restartBrowser,
      timeOut
    );
  ...
  async restartBrowser() {
    ...
    this._driver = await PuppeteerHandler_Browser.createPuppeteerObject_Browser(
      this._languageSavedValue
    );

    for (const pageId of Object.keys(this._pageList)) {
      if (this._pageList[pageId]) {
        const page = await this.createPuppeteerObject_Page(
          this._pageList[pageId].timeOut
        );
